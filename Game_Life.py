        ...
        for j in range(field_width):
            ...
            for i_shift in range(-1, 2):
                for j_shift in range(-1, 2):
                    if (canvas.gettags(cell_matrix[addr(i + i_shift, j + j_shift)])[0] == 'vis'
                        and (i_shift != 0 or j_shift != 0)):
                        k += 1		
...
win_width = 640
win_height = 480
config_string = "{0}x{1}".format(win_width + 5, win_height + 32)
fill_color = "green"
...
bot_bord = win_height // cell_size * (cell_size)
right_bord = win_width // cell_size * (cell_size)

canvas.create_line(2, 2, 2, bot_bord)
canvas.create_line(2, 2, right_bord, 2)

for j in range(field_height + 1):
    canvas.create_line(0, j*cell_size, right_bord, j*cell_size)
for i in range(field_width + 1):
    canvas.create_line(i*cell_size, 0, i*cell_size, bot_bord)

cell_matrix = []
for i in range(field_height):
    for j in range(field_width):
        square = canvas.create_rectangle(2 + cell_size*j, 2 + cell_size*i,
...
frame = Frame(root)
btn1 = Button(frame, text='Next step', command = step)
